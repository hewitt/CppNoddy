...
int main()
{
  ...
  cout << "\n";
  cout << "   ================ MORE SIMPLE 2X2 TESTS ===========\n";
  {
    //
    // SOLVE A SMALL "Sparse"(!) 2X2 REAL SYSTEM with MUMPS
    //
    ...
    std::cout << "\n";
  #ifdef MUMPS_SEQ
    std::cout << " Simple 2x2 double sparse system configured for MUMPS.\n";
    SparseLinearSystem<double> small_system( &A, &B, "mumps_seq" );
  #else
    std::cout << " Simple 2x2 double sparse system configured for native Gauss Jordan routine.\n";
    SparseLinearSystem<double> small_system( &A, &B, "native" );
  #endif
    ...
    {
      std::cout << " Simple 2x2 double sparse solver works.\n";
    }
  }

  {
    //
    // SOLVE A SMALL "Sparse"(!) 2X2 COMPLEX SYSTEM with MUMPS
    //
    ...
    std::cout << "\n";
    #ifdef MUMPS_SEQ
      std::cout << " Simple 2x2 COMPLEX sparse system configured for MUMPS.\n";
      SparseLinearSystem<D_complex> small_system( &A, &B, "mumps_seq" );
    #else
      std::cout << " Simple 2x2 COMPLEX sparse system configured for native Gauss Jordan routine.\n";
      SparseLinearSystem<D_complex> small_system( &A, &B, "native" );
    #endif
    ...
    {
      std::cout << " Simple 2x2 complex sparse solver works.\n";
    }
  }
  ...
  {
    //
    // SOLVE A SMALL "Sparse"(!) 2X2 double SYSTEM with MUMPS & re-solving
    //
    SparseMatrix<double> A( 2, 2 );
    DenseVector<double> B( 2, 0.0 );
    A( 0, 0 ) = 1.;
    ...
    std::cout << "\n";
    std::cout << " Simple 2x2 DOUBLE sparse system configured for MUMPS (with re-solve).\n";
    SparseLinearSystem<double> small_system( &A, &B, "mumps_seq" );

    try
    {
      small_system.solve();
      // try a re-solve with a new RHS
      B[ 0 ] = 10.;
      B[ 1 ] = 22.;
      small_system.solve_using_factorization();
    }
    catch ( std::runtime_error )
    {
      cout << " \033[1;31;48m  * FAILED THROUGH EXCEPTION BEING RAISED \033[0m\n";
    ...
    if ( B.inf_norm() > tol )
    {
      std::cout << "\033[1;31;48m Simple 2x2 double sparse system was not re-solved correctly\033[0m\n";
      std::cout << " residual vector's inf_norm = " << B.inf_norm() << "\n";
      failed = true;
    }
    else
    {
      std::cout << " Simple 2x2 double sparse re-solve works.\n";
    }
  }
  #endif
