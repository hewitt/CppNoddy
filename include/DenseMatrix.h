...
/// an STL Vector of DenseVectors, inheriting from Matrix_base.
...
namespace CppNoddy
{
  ...
  class DenseMatrix : public Matrix_base<_Type>
  {
  public:
  ...
  private:
    ...
    std::vector< DenseVector<_Type> > MATRIX;
  ...
  //    template <typename _Type>
  //    inline void DenseMatrix<_Type>::row_swap( const std::size_t& row1,
  //                                           const std::size_t& row2 )
  //    {
  //        // actually do the swap, rather than keep a row permutation vector.
  //        std::swap<DenseVector<_Type> > ( MATRIX[ row1 ], MATRIX[ row2 ] );
  //    }
