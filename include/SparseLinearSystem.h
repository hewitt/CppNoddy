...
namespace CppNoddy
{
  ...
  private:
    ...
    throw ExceptionExternal( problem );
      ...
      int* rows;
      int* cols;
      if ( !(storage = doubleMalloc(nnz)) ) ABORT("Malloc fails for storage[].");
      if ( !(cols = intMalloc(nnz)) ) ABORT("Malloc fails for rows[].");
      if ( !(rows = intMalloc(m+1)) ) ABORT("Malloc fails for cols[].");
