...
namespace CppNoddy {
  ...
  Equation_2matrix<_Type, _Xtype >::Equation_2matrix(const unsigned& order) :
    ...
    MATRIX1_AT_LAST_STATE = DenseMatrix<_Type>(order, order, 0.0);
  ...
  template <typename _Type, typename _Xtype>
  void Equation_2matrix<_Type, _Xtype >::update(const DenseVector<_Type> &state) {
  ...
  template <typename _Type, typename _Xtype>
  void Equation_2matrix<_Type, _Xtype>::get_jacobian_of_matrix1_mult_vector(const DenseVector<_Type> &state, const DenseVector<_Type> &vec, DenseMatrix<_Type> &h) const {
    ...
    DenseMatrix<_Type> copy_of_matrix(MATRIX1_AT_LAST_STATE);
    ...
    for(std::size_t i = 0; i < this -> ORDER_OF_SYSTEM; ++i) {
      ...
      copy_of_state[ i ] -= this -> DELTA;
      copy_of_matrix.sub(MATRIX1_AT_LAST_STATE);
