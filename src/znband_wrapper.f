...
c-----------------------------------------------------------------------
c
      subroutine znband( d , z, ldz, sigma,
     &                 n, ab, mb, lda, fac, kl, which,
     &                 nev, resid, ncv, v, ldv, iparam,
     &                 info )
c
c     %------------------%
c     | Scalar Arguments |
...
c     %-------------------------------------------%
c
      complex*16 workev( 2*ncv ), workd( 3*n ),
     &     workl( 3*ncv**2 + 5*ncv )
      integer ku, lworkl, iwork( n )
      Double precision tol, rwork( n )
      Logical          select(ncv)
