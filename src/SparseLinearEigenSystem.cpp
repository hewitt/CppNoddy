...
namespace CppNoddy
{
  ...
  template <typename _Type>
  void SparseLinearEigenSystem<_Type>::set_target( _Type target )
  {
    SHIFT = target;
  }
  ...
  void SparseLinearEigenSystem<_Type>::set_order( std::string order_string )
  {
    ...
    if ( order_string == "EPS_TARGET_MAGNITUDE" ) { ORDER=(EPSWhich)7; }
    if ( order_string == "EPS_TARGET_REAL" ) { ORDER=(EPSWhich)8; }
    if ( order_string == "EPS_TARGET_IMAGINARY" ) { ORDER=(EPSWhich)9; }
  ...
  //template <>
  //void SparseLinearEigenSystem<double>::eigensolve()
  //{
    //std::string problem;
    //problem = "The SparseLinearEigenSystem::eigensolve method has been called\n";
    //problem += "for a type <double> matrix. The SLEPc library has to be built separately\n";
    //problem += "for double and complex. At the moment CppNoddy assumes a complex build.\n";
...
//#ifdef DEBUG
    ...
    EPSGetType(eps,&type);
    PetscPrintf(p_LIBRARY -> get_Comm(),"[DEBUG] Solution method: %s\n\n",type);
    PetscPrintf(p_LIBRARY -> get_Comm(),"[DEBUG] Number of requested eigenvalues: %D\n",NEV);
    EPSGetTolerances(eps,&tol,&maxit);
    PetscPrintf(p_LIBRARY -> get_Comm(),"[DEBUG] Stopping condition: tol=%.4g, maxit=%D\n",(double)tol,maxit);
    /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
//#endif
  ...
  //template <>
  //void SparseLinearEigenSystem<double >::eigensolve_slepc()
  //{
//#ifndef SLEPC
    //std::string problem;
    //problem = "The SparseLinearEigenSystem::eigensolve method has been called\n";
