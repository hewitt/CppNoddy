...
namespace Example
{
  ...
  public:
    ...
    double FnComp_X( const double& x ) const
    {
      return BX + x - BX*exp(-(x - this -> LEFT)/CX );
    }
    double FnComp_Xd( const double& x ) const
    {
      return 1 + BX*exp(-(x - this -> LEFT)/CX )/CX;
    }
    double FnComp_Xdd( const double& x ) const
    {
      return - BX*exp(-(x - this -> LEFT)/CX )/(CX*CX);
    }
    // the computational y coord as a function of physical y
    double FnComp_Y( const double& y ) const
    {
      return BY + y - BY*exp(-(y - this -> BOTTOM)/CY );
    }
    double FnComp_Yd( const double& y ) const
    {
      return 1 + BY*exp(-(y - this -> BOTTOM)/CY )/CY;
    }
    double FnComp_Ydd( const double& y ) const
    {
      return - BY*exp(-(y - this -> BOTTOM)/CY )/(CY*CY);
    }
#endif
...
int main()
{
  ...
  const std::size_t NZ( 401 );
  const std::size_t NY( 401 );
  //
  //
